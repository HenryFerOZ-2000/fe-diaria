...
class StreakController extends ChangeNotifier {
  ...
  Map<String, bool> activeDaysMap = const {};
  ...
  void reloadFromFirestore({bool forceUpdate = false}) {
    if (forceUpdate) {
      ...
      debugPrint(
        '[StreakController] 🔄 Force reload from Firestore (will allow popup)',
      );
  ...
  void _loadFromFirestore({bool forceUpdate = false}) {
    ...
    if (uid == null) {
      debugPrint(
        '[StreakController] No user authenticated, loading local streak',
      );
      _loadGuestStreak(forceUpdate: forceUpdate);
      return;
    }

    debugPrint(
      '[StreakController] Loading streak from Firestore for uid=$uid (forceUpdate=$forceUpdate)',
    );
    ...
    _spiritualStatsService
        .getStats()
        .then((stats) {
          final previousTotal = totalDays;
          debugPrint(
            '[StreakController] Initial stats loaded: currentStreak=${stats.currentStreak}, previousTotal=$previousTotal, bestStreak=${stats.bestStreak}, isInitialLoad=$_isInitialLoad',
          );
          ...
          if (forceUpdate && totalDays > previousTotal) {
            debugPrint(
              '[StreakController] 🔥 Force update detected streak increase: $previousTotal -> $totalDays',
            );
    ...
    _statsSubscription = _spiritualStatsService.statsStream().listen(
      (stats) {
        final previousTotal = totalDays;
        debugPrint(
          '[StreakController] ⚡ Stats updated from stream: currentStreak=${stats.currentStreak}, previousTotal=$previousTotal, bestStreak=${stats.bestStreak}, isInitialLoad=$_isInitialLoad',
        );
        _updateFromStats(stats, isInitialLoad: _isInitialLoad);
        ...
        if (!_isInitialLoad && totalDays > previousTotal) {
          debugPrint(
            '[StreakController] 🔥 Stream detected streak increase: $previousTotal -> $totalDays',
          );
          _checkAndShowPopup(previousTotal, totalDays);
        }
  ...
  Future<void> _loadGuestStreak({bool forceUpdate = false}) async {
    final previousTotal = totalDays;
    final state = await StreakService(StorageService()).resetIfNeeded();
    totalDays = state.current;
    bestDays = state.best;
    lastActiveDate = state.lastDateYmd;
    if (state.lastDateYmd != null && state.current > 0) {
      activeDaysMap = {state.lastDateYmd!: true};
      _updateWeekDays(activeDaysMap);
    }
    notifyListeners();
    if (forceUpdate && totalDays > previousTotal) {
      _checkAndShowPopup(previousTotal, totalDays);
  ...
  void _updateFromStats(SpiritualStats stats, {bool isInitialLoad = false}) {
    ...
    if (changed) {
      debugPrint(
        '[StreakController] 🔄 Updating totalDays from $totalDays to $newTotalDays (isInitialLoad=$isInitialLoad)',
      );
    ...
    activeDaysMap = stats.activeDaysMap;
    ...
    if (changed) {
      debugPrint(
        '[StreakController] ✅ Notified listeners, totalDays is now $totalDays',
      );
    }
  }
  ...
  Future<void> _checkAndShowPopup(int previousTotal, int newTotal) async {
    try {
      ...
      if (shouldShow) {
        debugPrint(
          '[StreakController] 🔥 Streak increased from $previousTotal to $newTotal, showing popup (lastPopupDate=$lastPopupDate, lastPopupStreak=$lastPopupStreak)',
        );
        ...
        debugPrint(
          '[StreakController] ⏭️ Skipping popup: already shown today for streak $newTotal (lastPopupDate=$lastPopupDate, lastPopupStreak=$lastPopupStreak)',
        );
      }
    } catch (e) {
  ...
  String _getTodayDateString() {
    ...
    debugPrint(
      '[StreakController] 📅 Today date string (local): $y-$m-$d (device timezone)',
    );
    return '$y-$m-$d';
  }
  ...
  void _updateWeekDays(Map<String, bool> activeDaysMap) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      debugPrint(
        '[StreakController] Day $i (${updated[i].label}): date=$ymd, weekday=${date.weekday}, isTodayOrPast=$isTodayOrPast, completed=$completed',
      );

      updated[i] = StreakDay(label: updated[i].label, completed: completed);
    }
